  ...
  "BrowserFactory" should {
    ...
    "return chromeOptions with default options" in new Setup {
      ...
      options
        ...
        .toString                                 shouldBe "{args=[start-maximized], excludeSwitches=[enable-automation], extensions=[]}"
    ...
    "return userBrowserOptions for Chrome" in new Setup {
      ...
      options
        .asMap()
        .get("goog:chromeOptions")
        .toString                                 shouldBe "{args=[start-maximized, headless], excludeSwitches=[enable-automation], extensions=[]}"
      options.asMap().getOrDefault("proxy", None) shouldBe None
    }
    ...
    "return firefoxOptions with javascript disabled when disable.javascript is true" in new Setup {
      System.setProperty("disable.javascript", "true")
      val options: FirefoxOptions = browserFactory.firefoxOptions(None)
      options
        .asMap()
        .get("moz:firefoxOptions")
        .toString should include("{prefs={javascript.enabled=false}}")
    }
